// stripTerminalControls — the journal's terminal-safe text contract.
// Persisted note text is replayed raw to a terminal on every `journal show`;
// any C0/C1/ESC/DEL byte that survives becomes a live control sequence
// (window retitle, cursor moves, restyled output). Control characters are
// built with fromCharCode so no raw control bytes live in this source file.

import { describe, test, expect } from "bun:test";
import { stripTerminalControls } from "../sanitize.js";

const ESC = String.fromCharCode(0x1b);
const BEL = String.fromCharCode(0x07);
const DEL = String.fromCharCode(0x7f);
const CSI_C1 = String.fromCharCode(0x9b); // single-byte C1 CSI
const ST_C1 = String.fromCharCode(0x9c); // C1 string terminator
...
describe("stripTerminalControls", () => {
  ...
  test("folds tab/newline/CR runs to one space (word separation survives)", () => {
  ...
  test("an all-control string strips to whitespace-or-empty", () => {
    expect(stripTerminalControls(`${ESC}${BEL}${DEL}`).trim()).toBe("");
  });
});
