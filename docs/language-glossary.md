...
names, `auto`/`manual`, `crypto`/`bound`), JSON object keys, file paths, env vars (`NO_COLOR`, `ICHING_HOME`),
...
| `crypto` | entropy | crypto | 機器 (crypto) | 机器 (crypto) | domain term — canonical hint kept in label (row label 隨機源/随机源 "Entropy"); plain local machine entropy, no ritual claim |
| `bound` | entropy | bound | 繫於心念 (bound) | 系于心念 (bound) | domain term — canonical hint kept in label; honest-provenance register ("tied to the intention", 繫→系 / 於→于 per the audited conversion), never an efficacy claim |
...
- **`祐→佑` reviewed and KEPT:** an initial pass removed this mapping, but a second-opinion
  cross-check showed simplified 周易 editions conventionally print 自天佑之 and 祐/佑 are
  interchangeable for 上天保佑 — so the audited 祐→佑 stays (consistent with the table's
  other convention-over-OpenCC deviations 咷→啕 / 遯→遁). zh-Hans renders 自天佑之 (hex 14).
